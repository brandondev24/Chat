...
我们提供了一个精选示例，用于快速验证环境配置和模型加载是否正常：
...
你可以使用仓库中提供的官方 wrapper 启动 Gradio Web Demo。  
启动后，脚本会在终端中打印本地访问地址。请在浏览器中打开该地址，即可与模型进行交互。

更多使用细节、支持的任务类型以及额外示例，请参考官方 GitHub 仓库：
...
## 📜 开源协议
- **模型权重**：采用 CC BY-NC 4.0 许可证发布，仅限非商业用途。
