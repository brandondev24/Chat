...
## 🛠️ 使用指南
运行模型需配套 SenseNova-Vision 推理代码与各任务专用后处理脚本。以下示例默认已完成项目代码安装、并从Hugging Face下载完整模型仓库。
...
公开数据集标注尽可能直接转换使用；对于监督信息不足的场景，补充人工构建与自动生成标注，提升样本多样性。完整数据源清单、提示词模板、标注转换规则、数据集构建流程详见配套数据文档。
...
## 📜 开源协议
SenseNova-Vision 预发布版本完整许可协议将在正式公开发布前更新。同时使用时请遵守底层基础模型、训练数据集、第三方评测工具各自对应的开源许可条款。
